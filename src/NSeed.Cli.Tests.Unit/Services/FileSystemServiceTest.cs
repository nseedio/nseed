...
namespace NSeed.Cli.Tests.Unit.Services
{
    public class FileFixture : IDisposable
    {
        ...
        private readonly List<(string Directory, string File)> testSlnPaths = new List<(string Directory, string File)>
        {
            ...
            ($@"{RootDirectory}Projects\", "ProjectDelta.csproj"),
            ($@"{RootDirectory}EfCore\", "ProjectAlfa.csproj"),
            ($@"{RootDirectory}EfCore\", "ProjectBeta.csproj"),
        };
...
#pragma warning restore xUnit1019 // MemberData must reference a member providing a valid data type
        ...
        public class GetProjectPath : IClassFixture<FileFixture>
        {
            public static IEnumerable<object[]> ValidPaths => new List<object[]>
            {
                new object[] { $@"{FileFixture.RootDirectory}Projects" },
                new object[] { $@"{FileFixture.RootDirectory}Projects\ProjectDelta" },
                new object[] { $@"{FileFixture.RootDirectory}Projects\ProjectDelta.csproj" },
                new object[] { $@"{FileFixture.RootDirectory}EfCore" }
            };
            ...
            public static IEnumerable<object[]> InvalidPaths => new List<object[]>
            {
                ...
                new object[] { $@"Test/SubTest/SubSubTest", Resources.Info.SearchNSeedProjectPathErrors.Instance.FilePathDirectoryDoesNotExist },
                ...
                new object[] { $@"C:/Test238e9324/Test.sln", Resources.Info.SearchNSeedProjectPathErrors.Instance.FilePathDirectoryDoesNotExist },
                new object[] { $@"{FileFixture.RootDirectory}Fit\Fit.Web", Resources.Info.SearchNSeedProjectPathErrors.Instance.InvalidFile }
            };
...
#pragma warning restore xUnit1019 // MemberData must reference a member providing a valid data type
        ...
        public class TryGetTemplatePath
        {
            ...
            {
                var service = new FileSystemService();
                var response = service.TryGetTemplate(FrameworkType.NETCoreApp, out Template template);
